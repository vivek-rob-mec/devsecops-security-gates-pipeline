"""Reviewed teaching content shared by the deck, PDF and offline handbook."""

REVIEWED = '2026-09-16'
SOURCES = {
 'cve': ('CVE identifier lifecycle', 'https://www.cve.org/about/Process'),
 'nvd': ('NVD vulnerability enrichment', 'https://nvd.nist.gov/general/FAQ-Sections/CVE-FAQs'),
 'trivy': ('Trivy: vulnerability matching', 'https://trivy.dev/docs/latest/scanner/vulnerability/'),
 'trivy-db': ('Trivy database build and metadata', 'https://github.com/aquasecurity/trivy-db'),
 'trivy-cron': ('Trivy database publication schedule', 'https://raw.githubusercontent.com/aquasecurity/trivy-db/main/.github/workflows/cron.yml'),
 'java-db': ('Trivy Java index', 'https://github.com/aquasecurity/trivy-java-db'),
 'java-cron': ('Java index publication schedule', 'https://raw.githubusercontent.com/aquasecurity/trivy-java-db/main/.github/workflows/cron.yml'),
 'db-config': ('Trivy database refresh controls', 'https://trivy.dev/docs/latest/configuration/db/'),
 'dc': ('Dependency-Check detection model', 'https://devguide.owasp.org/en/05-implementation/02-dependencies/01-dependency-check/'),
 'dc-cli': ('Dependency-Check update options', 'https://dependency-check.github.io/DependencyCheck/dependency-check-cli/arguments.html'),
 'gitleaks': ('Gitleaks detection and configuration', 'https://github.com/gitleaks/gitleaks'),
 'semgrep': ('Semgrep rule and analysis concepts', 'https://semgrep.dev/docs/writing-rules/glossary'),
 'semgrep-cli': ('Semgrep command and exit semantics', 'https://docs.semgrep.dev/cli-reference'),
 'codeql': ('CodeQL database and query model', 'https://docs.github.com/en/code-security/concepts/code-scanning/codeql/codeql-cli'),
 'sonar': ('SonarQube analysis overview', 'https://docs.sonarsource.com/sonarqube-server/analyzing-source-code/overview'),
 'checkov': ('Checkov configuration and graph checks', 'https://www.checkov.io/1.Welcome/What%20is%20Checkov.html'),
 'zap-base': ('ZAP baseline scope and exit codes', 'https://www.zaproxy.org/docs/docker/baseline-scan/'),
 'zap-full': ('ZAP full scan', 'https://www.zaproxy.org/docs/docker/full-scan/'),
 'zap-addons': ('ZAP add-ons and rules', 'https://www.zaproxy.org/docs/desktop/addons/'),
 'syft': ('Syft software inventory', 'https://github.com/anchore/syft'),
 'grype': ('Grype vulnerability scanning', 'https://github.com/anchore/grype'),
 'cosign': ('Cosign signature verification', 'https://docs.sigstore.dev/cosign/verifying/verify/'),
 'kyverno': ('Kyverno admission validation', 'https://kyverno.io/docs/policy-types/cluster-policy/validate/'),
 'falco': ('Falco runtime rules', 'https://falco.org/docs/concepts/rules/'),
 'epss': ('FIRST Exploit Prediction Scoring System', 'https://www.first.org/epss/'),
 'cvss': ('FIRST CVSS v4.0 specification', 'https://www.first.org/cvss/v4.0/specification-document'),
 'kev': ('CISA official KEV data mirror', 'https://github.com/cisagov/kev-data'),
 'cwe': ('MITRE Common Weakness Enumeration', 'https://cwe.mitre.org/about/index.html'),
 'osv': ('Open Source Vulnerabilities', 'https://osv.dev/'),
 'ssdf': ('NIST SP 800-218 SSDF v1.1', 'https://csrc.nist.gov/pubs/sp/800/218/final'),
 'slsa': ('SLSA v1.2 specification', 'https://slsa.dev/spec/v1.2/'),
}

SLIDES = []
def add(section, title, subtitle, cards, notes, sources=(), takeaway='', layout='cards'):
    SLIDES.append(dict(section=section, title=title, subtitle=subtitle,
                       cards=cards, notes=notes, sources=list(sources),
                       takeaway=takeaway, layout=layout))

add('START', 'DevSecOps, explained from first principles',
    'Understand the flow. Understand the findings. Make a defensible release decision.',
    ...
    takeaway='Three learning paths, one consistent explanation.')

add('START', 'Choose a path through the material', 'Read the main course in sequence; return to technical sections when implementing.',
    ...
    takeaway='A scanner finding, a policy decision and a production release are different events.')

add('01 FOUNDATIONS', 'The problem: one change can carry many risks', 'Example: a developer changes checkout in an online shop.',
    ...
    takeaway='Choose controls according to the risk and the object being inspected.')

add('01 FOUNDATIONS', 'The delivery vocabulary', 'Expand the terms before reading a pipeline diagram.',
    ...
    takeaway='This course uses CD to mean Continuous Delivery with an explicit release decision.')

add('01 FOUNDATIONS', 'The security vocabulary', 'Each acronym names a different activity.',
    ...
    takeaway='Tool names are products; SCA, SAST and DAST are categories of work.')

add('01 FOUNDATIONS', 'A vulnerability ID is not a risk score', 'CVE, CWE and CVSS answer different questions.',
    ...
    ('cve','cwe','cvss'), takeaway='No CVE does not mean no security issue.')

add('01 FOUNDATIONS', 'Prioritize with evidence and context', 'Severity, exploitation and business impact are separate inputs.',
    ...
    ('cvss','epss','kev'), takeaway='A low prediction is not an exemption; a score needs application context.')

add('02 PIPELINE FLOW', 'The complete flow in four phases', 'Each phase produces evidence needed by the next.',
    ...
    takeaway='The diagram expresses dependencies; it does not require every scan to run serially.',layout='flow')

add('02 PIPELINE FLOW', 'Phase 1: define what must be protected', 'Owner: product team with security and platform engineering.',
    ...
    takeaway='Requirements become tests; a vague security goal cannot be verified.')

add('02 PIPELINE FLOW', 'Phase 2: verify the proposed change', 'Owner: developers; security engineers maintain rules and triage support.',
    ...
    takeaway='A report file existing is not proof that the scanner completed or found nothing.')

add('02 PIPELINE FLOW', 'Phase 3: identify and verify the artifact', 'Owner: build and platform engineering.',
    ...
    takeaway='Source scans and image scans inspect different inputs; retain both sets of evidence.')

add('02 PIPELINE FLOW', 'Phase 4: test, release and observe', 'Owner: application team and release operations.',
    ...
    takeaway='A passing release gate is a time-bounded decision about one artifact.')

add('03 DETECTION', 'Five working principles behind the tools', 'Ask what is being compared before asking which database is used.',
    ...
    takeaway='“Database” can mean advisories, a code model, an inventory or a rule bundle.')

add('03 DETECTION', 'Secret scanning: Gitleaks', 'Find credential-shaped material before it spreads.',
    ...
    ('gitleaks',), takeaway='Rotate a real leaked credential; deleting the string does not invalidate it.')

add('03 DETECTION', 'SCA: identify, then match', 'Software Composition Analysis starts with an accurate inventory.',
    ...
    takeaway='A missed component cannot receive a reliable vulnerability verdict.')

add('03 DETECTION', 'Worked example: an affected version range', 'Illustrative package and advisory; these are not real vulnerability records.',
    ...
    takeaway='Name + ecosystem + version + origin determine which advisory applies.')

add('03 DETECTION', 'Trivy: source and image vulnerabilities', 'One product, several scanners; enable the intended scope explicitly.',
    ...
    ('trivy',), takeaway='Check which scanner ran and which packages it recognized.')

add('03 DETECTION', 'Dependency-Check, OSV and Grype', 'Alternative matching systems share a goal but differ in identity and data.',
    ...
    ('dc','osv','grype'), takeaway='Compare evidence and coverage when tools disagree; do not just count alerts.')

add('04 INTELLIGENCE', 'How a disclosure becomes a scanner finding', 'There are several independent delays between discovery and detection.',
    ...
    ('cve','nvd'), takeaway='Discovery time, public disclosure, database refresh and your next scan are different clocks.',layout='flow')

add('04 INTELLIGENCE', 'When Trivy updates its data', 'Verified upstream schedules; actual local freshness depends on metadata and execution.',
    ...
    ('trivy-db','trivy-cron','java-db','java-cron'), takeaway='Measure the database actually used, not just the provider’s nominal schedule.')

add('04 INTELLIGENCE', 'Refresh intervals across other tools', 'Some tools refresh data; others need a new rule pack or executable.',
    ...
    ('dc-cli','epss','kev','zap-addons'), takeaway='Record both tool version and intelligence or rules revision.')

add('04 INTELLIGENCE', 'Freshness is an operational control', 'Suggested operating policy, not a product default or universal requirement.',
    ...
    ('db-config',), takeaway='No update + no finding can still mean “insufficient evidence.”')

add('03 DETECTION', 'SAST: follow data through code', 'Static Application Security Testing looks for unsafe program behavior.',
    ...
    takeaway='Inspect the source-to-sink evidence before deciding whether the finding is real.')

add('03 DETECTION', 'Semgrep: executable rules over code', 'Rules define what the engine should recognize.',
    ...
    ('semgrep','semgrep-cli'), takeaway='A successful process can contain findings unless failure behavior is configured.')

add('03 DETECTION', 'CodeQL and SonarQube use different models', 'A database can represent your code rather than public vulnerabilities.',
    ...
    ('codeql','sonar'), takeaway='Confirm analysis completion and the evaluated policy, not just report upload.')

add('03 DETECTION', 'IaC scanning: compare desired state to policy', 'Infrastructure as Code can create exposure before the application starts.',
    ...
    ('checkov',), takeaway='Configuration scanning predicts risk in declared infrastructure; verify deployed reality too.')

add('03 DETECTION', 'Containers: inspect what actually ships', 'An application can be clean while its base image is vulnerable.',
    ...
    takeaway='An image scan is not a runtime monitor or a complete hardening review.')

add('03 DETECTION', 'SBOM: inventory for today and tomorrow', 'Software Bill of Materials records component identity and relationships.',
    ...
    ('syft',), takeaway='An inventory enables vulnerability analysis; it is not a clean bill of health.')

add('03 DETECTION', 'Signing and provenance establish origin', 'Verify both cryptographic integrity and the expected identity.',
    ...
    ('cosign','slsa'), takeaway='Signed does not mean safe; verified expected origin is one release condition.')

add('03 DETECTION', 'DAST: inspect the running application', 'Dynamic Application Security Testing observes requests and responses.',
    ...
    takeaway='A clean scan of a login page says little about the authenticated application.')

add('03 DETECTION', 'ZAP: baseline is not a full active test', 'ZAP rules arrive through the installed tool and add-ons.',
    ...
    ('zap-base','zap-full','zap-addons'), takeaway='State the scan mode, authenticated coverage and outcome in every DAST report.')

add('03 DETECTION', 'API security needs business-aware assertions', 'Example: order access must remain within a customer account.',
    ...
    takeaway='Two authenticated users are often more useful than one anonymous scan.')

add('03 DETECTION', 'Admission control: prevent a bad deployment', 'Kubernetes admission evaluates requests before the cluster accepts them.',
    ...
    ('kyverno',), takeaway='A policy file in the repository does nothing until it is installed and enforced.')

add('03 DETECTION', 'Runtime detection: notice suspicious behavior', 'Falco inspects events after software starts running.',
    ...
    ('falco',), takeaway='Detection requires an operational response path; an unowned alert is incomplete.')

add('05 FINDINGS TO ACTION', 'How to read a vulnerability report', 'Follow one result from package identity to remediation.',
    ...
    takeaway='A useful finding explains what was matched, where, why and what to do next.')

add('05 FINDINGS TO ACTION', 'False positives, false negatives and unknowns', 'Accuracy includes knowing the limits of the inspection.',
    ...
    takeaway='Reduce noise through evidence and scoped rules, not broad exclusions.')

add('05 FINDINGS TO ACTION', 'Turn scanner output into an explicit gate', 'This project uses a conservative, simple executable baseline.',
    ...
    takeaway='Document implemented behavior separately from the desired production architecture.')

add('05 FINDINGS TO ACTION', 'The release decision must fail closed', 'Missing, stale or mismatched evidence prevents approval.',
    ...
    takeaway='A gate that always prints success is a missing control.')

add('05 FINDINGS TO ACTION', 'Exceptions need a lifecycle', 'An exception is a documented risk decision with a limit.',
    ...
    takeaway='An exception does not remove the finding or the obligation to remediate it.')

add('05 FINDINGS TO ACTION', 'What belongs in the release evidence package', 'Keep enough context to repeat and defend the decision.',
    ...
    takeaway='Evidence is useful only when its origin, completeness and artifact binding can be trusted.')

add('06 PROJECT PRACTICE', 'What this repository actually provides', 'A teaching reference with runnable wrappers and incomplete deployment integrations.',
    ...
    takeaway='Use the implementation guide to turn the reference into a tested application pipeline.')

add('06 PROJECT PRACTICE', 'Local use: prerequisites and first checks', 'Run Bash wrappers in Git Bash, Linux or a configured WSL environment.',
    [('PREPARE', 'Install reviewed Git, Gitleaks, Trivy and Semgrep versions. Allow required advisory access.'),
     ('EXECUTE', 'bash scripts/preflight.sh\nbash scripts/secret-scan.sh\nbash scripts/sca-scan.sh'),
     ('CONTINUE', 'bash scripts/sast-scan.sh\nbash scripts/iac-scan.sh\nInspect reports/generated/ and exit statuses.')],
    ...
    takeaway='See docs/implementation/quickstart.md for commands, outputs and troubleshooting.')

add('06 PROJECT PRACTICE', 'A stack changes inputs, not the control goals', 'Use the same reasoning across application languages.',
    ...
    takeaway='Scan the resolved dependencies and the final deliverable; validate the supported formats.')

add('06 PROJECT PRACTICE', 'CI templates need explicit setup', 'Continuous Integration platforms differ in syntax and trust configuration.',
    ...
    takeaway='A copied YAML file is an integration starting point, not proof of enforcement.')

add('06 PROJECT PRACTICE', 'Corrections made during this review', 'Changes address misleading pass results and inconsistent scope.',
    ...
    takeaway='See docs/PROJECT_REVIEW.md for the issue list, changes and remaining integration work.')

add('06 PROJECT PRACTICE', 'Troubleshooting by symptom', 'Resolve missing evidence before interpreting security results.',
    ...
    takeaway='“The tool ran” and “the intended scope was assessed” are separate checks.')

add('07 OPERATIONS', 'After release: a new CVE changes the answer', 'The artifact is unchanged; the available knowledge has changed.',
    ...
    takeaway='A security result expires as the artifact, environment or intelligence changes.')

add('07 OPERATIONS', 'Standards guide outcomes and assurance', 'This architecture is a reference; its exact sequence is not universally mandated.',
    ...
    ('ssdf','slsa'), takeaway='Use named versions and evidence-based mappings; avoid unsupported compliance claims.')

add('07 OPERATIONS', 'Adopt in small, verifiable increments', 'Suggested engineering sequence; adapt it to application risk.',
    ...
    takeaway='One reliable, owned control is more useful than several unverified report generators.')

add('08 REVIEW', 'Check your understanding', 'Try answering before opening the explanations in the notes.',
    ...
    takeaway='If you can explain the evidence behind a decision, you understand the pipeline.')

add('08 REVIEW', 'What a defensible release can explain', 'Five questions connect the entire course.',
    ...
    takeaway='Reviewed source → tested artifact → verified release → continuous response.')

# Keep section numbering in reading order while preserving lesson order within each section.
SLIDES.sort(key=lambda item: '00' if item['section']=='START' else item['section'][:2])
